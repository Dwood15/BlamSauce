	...
	namespace Players
	{
		...
		struct s_player_yelo_server_data
		{
			...
			struct _rcon_access {
				...
				int16 number_of_bad_passwords;		// Number of times the player_update has given a bad password in a given threshold of time
			}rcon_access;
		...
		struct s_player_datum : Memory::s_datum_base
		{
			struct s_game_engine_state_message
			{
				...
				int32 target_time; // timer used to fade in the target player_update name
			};
