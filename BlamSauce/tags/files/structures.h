	...
	private:
		...
		bool SingleFileCreate(tag group_tag, cstring name) {
			...
			if (blam::tag_file_read_only(group_tag, name)) {
				YELO_ERROR_FAILURE("couldn't overwrite read-only tag file '%s'", blam::file_reference_get_name(single_file_reference, Flags::_name_full_path_flags, debug_filename));

				return false;
			}

			if (!blam::file_create(file_reference)) {
				YELO_ERROR_FAILURE("couldn't create tag file '%s'", blam::file_reference_get_name(single_file_reference, Flags::_name_full_path_flags, debug_filename));
