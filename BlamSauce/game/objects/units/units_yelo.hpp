	...
	namespace Objects {
		...
		void __cdecl BipedSeatedMelee(const datum_index unit_index) {
			...
			if (weapon_index.IsNull()) {
				...
				auto animation_index = GetSeatWeaponAnimation(animations_definition, biped_datum->unit.animation.seat_index, biped_datum->unit.animation.seat_weapon_index,
				...
				animation_index = blam::animation_choose_random_permutation_internal(Enums::_animation_update_kind_affects_game_state, unit_definition->object.references.animations.tag_index,
																											animation_index);
		...
		///
		/// <param name="unit_index">	Datum index of the unit. </param>
		void Units::UnitCausePlayerSeatedMelee(const datum_index unit_index);
	}
};
