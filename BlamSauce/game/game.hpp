...
namespace Yelo {
	...
	namespace GameState {
		struct s_game_options {
			...
			// 0x4 short game_simulation; // _game_simulation_local is default

			UNKNOWN_TYPE(long); // never see this referenced or explicitly set. due to memset, this will always be set to zero
			UNKNOWN_TYPE(short); // never see this referenced besides in game_options_new code where it's set to 0, even after a memset call. highly likely to be an enum field, albeit unused
