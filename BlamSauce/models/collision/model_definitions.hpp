...
namespace Yelo
{
	...
	namespace TagGroups
	{
		...
		struct s_damage_resistance // or s_model_damage_info
		{
			...
			TAG_FIELD(short, indirect_damage_material, collision_model_material);
			unsigned short : 16;
		...
		{
			TAG_FIELD(short, node, collision_model_node);
			unsigned short : 16;
			TAG_PAD(long, 3);
		...
		{
			TAG_FIELD(tag_string, name);
			TAG_FIELD(short, region, collision_model_region);
			TAG_FIELD(short, parent_node, collision_model_node);
			TAG_FIELD(short, next_sibling_node, collision_model_node);
			TAG_FIELD(short, first_child_node, collision_model_node);
			TAG_PAD(tag_block, 1);
