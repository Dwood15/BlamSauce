...
namespace Yelo::blam {
	...
	void *__cdecl debug_malloc(const size_t size, const bool fill_with_garbage, cstring file, const uint32 line);

	template <typename T>
	inline T *debug_new(const bool fill_with_garbage, cstring file, const uint32 line) {
		return CAST_PTR(T*, debug_malloc(sizeof(T) * 1, fill_with_garbage, file, line));
	}

	template <typename T>
	inline T *debug_new_array(const size_t count, const bool fill_with_garbage, cstring file, const uint32 line) {
		return CAST_PTR(T*, debug_malloc(sizeof(T) * count, fill_with_garbage, file, line));
	}

	void __cdecl debug_free(void *pointer, cstring file, const uint32 line);
	...
	void *__cdecl debug_realloc(void *pointer, const size_t new_size, cstring file, const uint32 line);
