...
namespace Yelo
{
	...
	namespace Objects
	{
		struct s_weapon_datum_network_data
		{
			...
			unsigned long : 32;
			unsigned long : 32;
			unsigned long : 32; // not used in the update...probably a real_vector3d (angular_velocity?)
			short magazine_rounds_totals[Enums::k_maximum_number_of_magazines_per_weapon];
			real age;
		...
		struct s_weapon_data
		{
			...
			struct s_magazine_state // '?' means IDK if its actually padding or there are values there. If there are, IDK their types (could be a boolean!)
			{
				...
				short reload_time_remaining;		// 0x2 in ticks
				short reload_time;					// 0x4 in ticks
				short rounds_unloaded;				// 0x6
				short rounds_loaded;				// 0x8
				short rounds_left_to_recharge;		// 0xA number of rounds left to apply to rounds_loaded (based on tag's rounds_recharged)
			...
			struct s_start_reload_data
			{
				...
				short starting_loaded_rounds[Enums::k_maximum_number_of_magazines_per_weapon];
			...
			unsigned char : 8;
			short ready_time;						// 0x23A in ticks
			real heat;								// 0x23C
			real age;								// 0x240
			real illumination_fraction;				// 0x244
			real integrated_light_power;			// 0x248
			unsigned long : 32;									// 0x24C unused
			datum_index tracked_object_index;		// 0x250
			PAD64;									// 0x254 unused
			short alt_shots_loaded;					// 0x25C
			unsigned short : 16;									// 0x25E
